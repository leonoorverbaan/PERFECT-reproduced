...
def main():
    ...
    def get_initial_weights(config, verbalizers):
        ...
        weight_init = torch.normal(mean=0.0, std=config.extra_embd_initializer_range,
                                   size=(num_extra_tokens, config.hidden_size))
    ...
    extra_embedding_weight = get_initial_weights(config,
                                                 verbalizers) if training_args.extra_tokens_init == 'verbalizers' else None
    ...
    if training_args.do_predict:
        logger.info("*** Predict ***")
        print("Now trying to predict... not sure if it will work")
		# print("Passing prediction step (in run_clm.py)..... Not managed to fix this. TODO: fix this")
