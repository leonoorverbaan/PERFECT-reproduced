...
class AbstractProcessor(abc.ABC):
    ...
    def get_tokenized_verbalizers(self, example=None):
       ...
       per example. In this function, as a default, we compute the static one."""
        ...
        self.tokenized_verbalizers = [[get_verbalization_ids(word=verbalizer, tokenizer=self.tokenizer)] for verbalizer
                                      in verbalizers]
        return self.tokenized_verbalizers
...
class MFRC(MR):
    ...
    def get_verbalizers(self):
        return ['Non-Moral' 'Purity' 'Thin Morality' 'Equality' 'Authority' 'Care'
                'Care,Loyalty' 'Loyalty' 'Proportionality' 'Purity,Authority'
                'Care,Equality,Proportionality' 'Authority,Proportionality'
                'Proportionality,Care,Equality' 'Loyalty,Proportionality'
                'Loyalty,Authority' 'Care,Authority' 'Care,Equality' 'Care,Purity'
                'Care,Proportionality' 'Care,Purity,Equality' 'Proportionality,Equality'
                'Purity,Care' 'Authority,Equality']
				
class MFTC(MR):
    name = "mftc"

    def get_verbalizers(self):
        return ['fairness','non-moral','purity','degradation','loyalty','care','cheating','betrayal','subversion','authority','harm']
        ...
        if not self.with_pattern:
            ...
            if self.mask_position is None:
                return [Text(text=example["premise"], shortenable=True),
                        Text(text=example["hypothesis"], shortenable=True), *masks], []
            else:
...
class CB(RTE):
    ...
    def get_prompt_parts(self, example, mask_length):
        if self.pattern_id == 4:
            ...
            masks = mask_length * [Text(text=self.mask_token)]
            return [premise, Text(text=' question: '), hypothesis, Text(text=' true, false or neither? answer:'),
                    *masks], []
...
class WiC(AbstractProcessor):
    ...
    def get_sentence_parts(self, example, mask_length):
        if not self.with_pattern:
            ...
            if self.mask_position is None:
                ...
                return self.get_parts_with_setting_masks(
                    part_0=[Text(text=example["word"]), Text(text=example["sentence1"], shortenable=True)],
    ...
    def get_prompt_parts(self, example, mask_length):
        ...
        if self.pattern_id == 1:
            return [sentence1, sentence2, Text(text='Does ' + word + ' have the same meaning in both sentences?'),
                    *masks], []
        if self.pattern_id == 2:
